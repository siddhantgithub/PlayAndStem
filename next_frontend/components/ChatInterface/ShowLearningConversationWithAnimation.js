...
import {
  CommonChapterEndBlock,
  QuizEndBlock,
  ConceptEndBlock,
} from "../../assets/lessons/ZacobiaMission/0_CommonModules";
import {
...
export default function LearningConversation(props) {
  ...
  const [
    ...
    state.speechVolume,
    state.updateSpeechVolume,
    state.isCairoMuted,
    state.updateCairoMuted,
    state.cairoVoice,
    state.updateCairoVoice,
    state.forwardSpeed,
    state.updateForwardSpeed,
  ]);
      ...
      if (arrayElem.type == "quizend") {
        setComponentArray((componentArray) => {
          ...
          return [
            ...componentArray,
            <ChatBotMessage
              message={"Quiz has ended. Your score is " + arrayElem.data + "%"}
              key={componentKey.current++}
            />,
          ];
        });
        ...
        <Grid item xs={3} md={3} lg={5}>
          ...
          <Stack direction="row" sx={{ mb: 1, mt: 2 }} alignItems="center">
            ...
            <IconButton aria-label="mute" onClick={muteButtonPress}>
              {isCairoMuted ? (
                <VolumeOffIcon sx={{ color: pink[500] }} />
              ) : (
                <VolumeUpIcon />
              )}
            </IconButton>
            <IconButton
              aria-label="rewind"
              onClick={rewindButtonClicked}
              disabled={forwardSpeed == CairoSpeedPossible.Slow}
            >
              {<FastRewindIcon />}
            </IconButton>
            <IconButton
              ...
              disabled={forwardSpeed == CairoSpeedPossible.Fast}
            >
              {<FastForwardIcon />}
            </IconButton>
            {/*<Select
